...
bool ClightingSprite::init(){
  ...
  //  this->getTexture()->setAliasTexParameters();
    ...
    //lighting source
    {
        ...
        lightingSrcSprite->setScale(0.25);
    ...
    //lighting dest
    {
        ...
        lightingDstSprite->setScale(0.25);
...
vector<ClineSeg*> ClightingSprite::splitSeg(ClineSeg*seg,int generationID)
{
   // CCLOG("seg->father branchID:%i",seg->branchID);
    ...
    //calculate branch
    float p=nPow(0.55,generationID-1);//0.6
...
void ClightingSprite::doJitter_reGenBranches(){
    ...
    //mark branch to be regenerated
    {
        ...
        for(int i=0;i<nbranch;i++){
            ...
            float p=//percentageOfJitterBranch+(0.8-percentageOfJitterBranch)/(nGeneration-1)*(branchSeg->branchStartGenerationID-1);
                    //1-nPow(1-percentageOfJitterBranch,branchSeg->branchStartGenerationID);
                   //percentageOfJitterBranch+0.1*(branchSeg->branchStartGenerationID-1);//
                    percentageOfJitterBranch;
        //    assert(branchSeg->branchStartGenerationID>=1);
            if(catchProbability(p)){
...
void ClightingSprite::update(float t){
   ...
   // CCLOG("nGeneration:%i",nGeneration);
    ...
    if(inDelayAfterFade==false){

        this->setOpacity(this->getOpacity()-dOpacity);
        if(this->getOpacity()-dOpacity<=dOpacity){//fade out finished
            inDelayAfterFade=true;
            delayAfterFadeCounter=0;
        }
    }else{
        this->setOpacity(0);
        delayAfterFadeCounter++;
        if(delayAfterFadeCounter>=delayAfterFade){//delay finished
            this->genLighting();
    ...
    for(int i=0;i<howManyJitterInOneFade;i++){
        ...
        if(this->getOpacity()>=value&&this->getOpacity()-dOpacity<value){
            doJitter();
            genMesh();
        }
    }
